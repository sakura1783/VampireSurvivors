...
public class TitlePopUp : MonoBehaviour
{
    ...
    //上記を配列に変更(ランキングをfor文で回せるようにするため)
    ...
    public void LoadRanking()
    {
        ...
        //List<(string playerName, int score)> playersDataList = PlayerPrefsUtility.LoadList<(string, int)>("PlayersDataList_Key");  //<= 不要。ここで取得しても、GameDataのListは空のまま。
        ...
        //上記の処理をfor文に変更
        for (int i = 0; i < GameData.instance.playersDataList.Count; i++)  //<= ループする回数はランキングとして保存されている回数しか回らないので、要素の数が何個であっても対応できる。
        {
            txtNames[i].text = GameData.instance.playersDataList[i].playerName;
            txtScores[i].text = GameData.instance.playersDataList[i].score.ToString();
        }
    }
